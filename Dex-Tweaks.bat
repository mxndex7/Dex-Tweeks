...
start https://github.com/mxndex7
echo         %c%If the page doesn't open automatically, visit:%u%
echo         %lime%https://github.com/mxndex7%u%
echo.
echo.
echo %c%══════════════════════════ PRESS ANY KEY TO CONTINUE ══════════════════════════%u%
pause >nul
...
echo %red%Ignoring checksums reduces package integrity protection.%u%
choice /C YN /M "Retry with --ignore-checksums? (Y/N)"
...
if errorlevel 1 (
    call :ConfirmChecksumBypass
    if errorlevel 1 (
        echo %yellow%Skipped checksum bypass for %name%.%u%
        echo.
        exit /b
    )
    echo %c%Retrying with checksum bypass...%u%
