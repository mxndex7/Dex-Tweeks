...
echo Dex Tweaks requires Administrator privileges to avoid partial or failed tweaks.
...
echo.
echo.
echo                                                 %lime% Developed by%u% %violet%Mendes%u%
echo.
...
echo.
echo.
echo                                %c%Dex%u%%white% Tweaks%u%%c% is a batch script that optimizes your system%u%
...
if errorlevel 12 goto GlobalSearch
if errorlevel 11 goto BenchmarkCenter
if errorlevel 10 goto HealthCenter
if errorlevel 9 goto BackupManager
if errorlevel 8 goto ChangeCenter
if errorlevel 7 goto AdvancedMenu
if errorlevel 6 goto PrivacyMenu
if errorlevel 5 goto WindowsMenu
if errorlevel 4 goto HardwareMenu
if errorlevel 3 goto TweaksMenu
if errorlevel 2 goto ProfilesCenter
if errorlevel 1 goto DashboardCenter
goto menu
...
echo.
echo                              %c%It will be available soon in a future update.%u%
echo.
echo.
echo.
echo.
...
pause >nul
if defined COMINGSOON_BACK (
    set "COMINGSOON_TARGET=%COMINGSOON_BACK%"
    set "COMINGSOON_BACK="
    goto %COMINGSOON_TARGET%
)
goto menu
...
echo      %c%to deliver a reliable Windows optimization toolkit. Every tweak has been%u%
echo      %c%carefully selected to maximize performance, responsiveness, and overall%u%
...
echo         %c%features added to Dex Tweaks. The complete changelog and%u%
...
if exist "%FIREFOX_PREF_DIR%" (
    (
        echo // Firefox Privacy Configuration - Generated by Dex Tweaks on %DATE% %TIME%
        ...
        echo pref^("app.update.enabled", true^);
        echo pref^("app.update.auto", true^);
        echo pref^("app.update.mode", 1^);
        echo pref^("app.update.service.enabled", true^);
        echo.
...
powercfg /changename 44444444-4444-4444-4444-444444444442 "Dex Laptop Performance" "Balanced performance plan optimized for gaming laptops" >nul 2>&1
...
rem GUID (not limited to a fixed range), identifies which ones are AMD adapters via
rem ProviderName, logs every candidate found, and lets the user pick when there is
rem more than one. Sets AMD_GPU_INDEX and AMD_GPU_CLASS_KEY on success (errorlevel 0).
rem On failure, cancellation, or an invalid choice it clears both variables, writes
rem no registry values, and returns errorlevel 1.
set "AMD_GPU_CLASS_KEY="
set "AMD_GPU_INDEX="
set "AMD_CAND_COUNT=0"
set "_amd_class=HKLM\SYSTEM\CurrentControlSet\Control\Class\{4d36e968-e325-11ce-bfc1-08002be10318}"
call :LogEvent "INFO" "AMD GPU detection started for :AMDGPU"
for /f "delims=" %%K in ('reg query "%_amd_class%" 2^>nul ^| findstr /r /i "\\[0-9][0-9][0-9][0-9]$"') do (
    set "_amd_scan_idx="
    for %%N in ("%%K") do set "_amd_scan_idx=%%~nN"
    set "_amd_scan_provider="
    for /f "tokens=2,*" %%a in ('reg query "%_amd_class%\!_amd_scan_idx!" /v ProviderName 2^>nul ^| findstr /i "ProviderName"') do set "_amd_scan_provider=%%b"
    if defined _amd_scan_provider (
        set "_amd_is_amd="
        echo(!_amd_scan_provider!| findstr /I /C:"Advanced Micro Devices" /C:"ATI Technologies" /C:"AMD" >nul && set "_amd_is_amd=1"
        if defined _amd_is_amd (
            set "_amd_scan_desc=Unknown"
            for /f "tokens=2,*" %%a in ('reg query "%_amd_class%\!_amd_scan_idx!" /v DriverDesc 2^>nul ^| findstr /i "DriverDesc"') do set "_amd_scan_desc=%%b"
            set "_amd_scan_devid=Unknown"
            for /f "tokens=2,*" %%a in ('reg query "%_amd_class%\!_amd_scan_idx!" /v MatchingDeviceId 2^>nul ^| findstr /i "MatchingDeviceId"') do set "_amd_scan_devid=%%b"
            if "!_amd_scan_devid!"=="Unknown" (
                for /f "delims=" %%a in ('powershell -NoProfile -Command "try { $b = (Get-ItemProperty -Path 'HKLM:\SYSTEM\CurrentControlSet\Control\Class\{4d36e968-e325-11ce-bfc1-08002be10318}\!_amd_scan_idx!' -Name 'HardwareInformation.AdapterString' -ErrorAction Stop).'HardwareInformation.AdapterString'; [Text.Encoding]::Unicode.GetString($b).TrimEnd([char]0) } catch { '' }" 2^>nul') do if not "%%a"=="" set "_amd_scan_devid=%%a"
            )
            set /a "AMD_CAND_COUNT+=1"
            set "AMD_CAND_!AMD_CAND_COUNT!_IDX=!_amd_scan_idx!"
            set "AMD_CAND_!AMD_CAND_COUNT!_DESC=!_amd_scan_desc!"
...
if "!AMD_CAND_COUNT!"=="0" (
    echo %red%No AMD GPU was found under the Display adapters registry class.%u%
    call :LogEvent "BLOCKED" "AMD GPU tweaks refused: no AMD adapter detected under !_amd_class!"
    exit /b 1
)
...
if "!AMD_CAND_COUNT!"=="1" (
    ...
    echo.
    echo %c%Multiple AMD GPUs were found. Select which one to optimize:%u%
    for /l %%N in (1,1,!AMD_CAND_COUNT!) do (
        call echo   [%%N] %%AMD_CAND_%%N_DESC%% ^(index %%AMD_CAND_%%N_IDX%%^)
    )
    echo.
    set "_amd_selected="
    ...
    set "_amd_valid_selection="
    for /l %%N in (1,1,!AMD_CAND_COUNT!) do if "!_amd_selected!"=="%%N" set "_amd_valid_selection=1"
    if not defined _amd_valid_selection (
        echo %red%Invalid selection "!_amd_selected!". No registry changes were made.%u%
        call :LogEvent "BLOCKED" "AMD GPU tweaks refused: invalid selection !_amd_selected!"
        exit /b 1
    )
)

call set "AMD_GPU_INDEX=%%AMD_CAND_!_amd_selected!_IDX%%"
call set "_amd_confirm_desc=%%AMD_CAND_!_amd_selected!_DESC%%"

set "_amd_reverify_provider="
for /f "tokens=2,*" %%a in ('reg query "%_amd_class%\!AMD_GPU_INDEX!" /v ProviderName 2^>nul ^| findstr /i "ProviderName"') do set "_amd_reverify_provider=%%b"
set "_amd_reverify_ok="
if defined _amd_reverify_provider echo(!_amd_reverify_provider!| findstr /I /C:"Advanced Micro Devices" /C:"ATI Technologies" /C:"AMD" >nul && set "_amd_reverify_ok=1"
if not defined _amd_reverify_ok (
    echo %red%The selected registry subkey no longer matches an AMD adapter. Aborting.%u%
    call :LogEvent "BLOCKED" "AMD GPU tweaks refused: re-validation of index !AMD_GPU_INDEX! failed"
    set "AMD_GPU_INDEX="
    exit /b 1
)
...
set "_wifi_pnp_found="
for /f "delims=" %%j in ('powershell -NoProfile -Command "Get-CimInstance -ClassName Win32_NetworkAdapter -ErrorAction SilentlyContinue | Where-Object { $_.NetEnabled -eq $true -and $_.AdapterTypeId -eq 9 } | Select-Object -ExpandProperty PNPDeviceID" 2^>nul') do (
    set "_wifi_pnp_found=1"
...
if not defined _wifi_pnp_found for /f "tokens=1* delims=" %%i in ('wmic path Win32_NetworkAdapter where "NetEnabled=true and AdapterTypeId=9" get PNPDeviceID /format:value ^| findstr "PNPDeviceID"') do (
...
set "_eth_speed_raw="
for /f "delims=" %%a in ('powershell -NoProfile -Command "Get-CimInstance -ClassName Win32_NetworkAdapter -ErrorAction SilentlyContinue | Where-Object { $_.NetConnectionStatus -eq 2 -and $_.AdapterTypeId -eq 0 } | Select-Object -First 1 -ExpandProperty Speed" 2^>nul') do set "_eth_speed_raw=%%a"
if not defined _eth_speed_raw for /f "tokens=2 delims==" %%a in ('wmic path win32_networkadapter where "NetConnectionStatus=2 and AdapterTypeId=0" get Speed /format:value 2^>nul ^| findstr "Speed"') do set "_eth_speed_raw=%%a"
if defined _eth_speed_raw (
...
powercfg -setactive scheme_current >nul 2>&1
set "_eth_pnp_found="
for /f "delims=" %%j in ('powershell -NoProfile -Command "Get-CimInstance -ClassName Win32_NetworkAdapter -ErrorAction SilentlyContinue | Where-Object { $_.NetEnabled -eq $true -and $_.AdapterTypeId -eq 0 } | Select-Object -ExpandProperty PNPDeviceID" 2^>nul') do (
    set "_eth_pnp_found=1"
    reg add "HKLM\SYSTEM\CurrentControlSet\Enum\%%j\Device Parameters" /v "SelectiveSuspendEnabled" /t REG_DWORD /d 0 /f >nul 2>&1
    reg add "HKLM\SYSTEM\CurrentControlSet\Enum\%%j\Device Parameters" /v "SelectiveSuspendOn" /t REG_DWORD /d 0 /f >nul 2>&1
...
if not defined _eth_pnp_found for /f "tokens=1* delims=" %%i in ('wmic path Win32_NetworkAdapter where "NetEnabled=true and AdapterTypeId=0" get PNPDeviceID /format:value ^| findstr "PNPDeviceID"') do (
    for /f "tokens=2 delims==" %%j in ("%%i") do (
        reg add "HKLM\SYSTEM\CurrentControlSet\Enum\%%j\Device Parameters" /v "SelectiveSuspendEnabled" /t REG_DWORD /d 0 /f >nul 2>&1
        reg add "HKLM\SYSTEM\CurrentControlSet\Enum\%%j\Device Parameters" /v "SelectiveSuspendOn" /t REG_DWORD /d 0 /f >nul 2>&1
        reg add "HKLM\SYSTEM\CurrentControlSet\Enum\%%j\Device Parameters" /v "FlowControl" /t REG_DWORD /d 3 /f >nul 2>&1
...
powercfg -setactive scheme_current >nul 2>&1
set "_all_pnp_found="
for /f "delims=" %%j in ('powershell -NoProfile -Command "Get-CimInstance -ClassName Win32_NetworkAdapter -ErrorAction SilentlyContinue | Where-Object { $_.NetEnabled -eq $true } | Select-Object -ExpandProperty PNPDeviceID" 2^>nul') do (
    set "_all_pnp_found=1"
    reg add "HKLM\SYSTEM\CurrentControlSet\Enum\%%j\Device Parameters" /v "SelectiveSuspendEnabled" /t REG_DWORD /d 0 /f >nul 2>&1
    reg add "HKLM\SYSTEM\CurrentControlSet\Enum\%%j\Device Parameters" /v "SelectiveSuspendOn" /t REG_DWORD /d 0 /f >nul 2>&1
...
if not defined _all_pnp_found for /f "tokens=1* delims=" %%i in ('wmic path Win32_NetworkAdapter where "NetEnabled=true" get PNPDeviceID /format:value ^| findstr "PNPDeviceID"') do (
    for /f "tokens=2 delims==" %%j in ("%%i") do (
        reg add "HKLM\SYSTEM\CurrentControlSet\Enum\%%j\Device Parameters" /v "SelectiveSuspendEnabled" /t REG_DWORD /d 0 /f >nul 2>&1
        reg add "HKLM\SYSTEM\CurrentControlSet\Enum\%%j\Device Parameters" /v "SelectiveSuspendOn" /t REG_DWORD /d 0 /f >nul 2>&1
        reg add "HKLM\SYSTEM\CurrentControlSet\Enum\%%j\Device Parameters" /v "DeviceSelectiveSuspended" /t REG_DWORD /d 0 /f >nul 2>&1
...
if "!CONN_TYPE!"=="WiFi" (
    ...
    if not defined _wifi_conn_pnp_found for /f %%a in ('wmic path win32_networkadapter where "NetConnectionStatus=2 and AdapterTypeId=9" get PNPDeviceID /format:value ^| findstr "PNPDeviceID"') do (
    ...
    if not defined _eth_conn_pnp_found for /f %%a in ('wmic path win32_networkadapter where "NetConnectionStatus=2 and AdapterTypeId=0" get PNPDeviceID /format:value ^| findstr "PNPDeviceID"') do (
        for /f "tokens=2 delims==" %%b in ("%%a") do (
            reg add "HKLM\SYSTEM\CurrentControlSet\Enum\%%b\Device Parameters" /v "FlowControl" /t REG_DWORD /d 3 /f >nul 2>&1
            reg add "HKLM\SYSTEM\CurrentControlSet\Enum\%%b\Device Parameters" /v "SpeedDuplex" /t REG_DWORD /d 0 /f >nul 2>&1
        )
...
echo %red%If Chrome, Firefox, or Edge are running, they will be closed and reopened%u%
...
set "mobo_manufacturer="
for /f "delims=" %%i in ('powershell -NoProfile -Command "(Get-CimInstance -ClassName Win32_BaseBoard -ErrorAction SilentlyContinue).Manufacturer" 2^>nul') do set "mobo_manufacturer=%%i"
if not defined mobo_manufacturer for /f "tokens=1,* delims==" %%a in ('wmic baseboard get manufacturer /value ^| find "="') do for /f "delims=" %%i in ("%%b") do set "mobo_manufacturer=%%i"

set "mobo_model="
for /f "delims=" %%i in ('powershell -NoProfile -Command "(Get-CimInstance -ClassName Win32_BaseBoard -ErrorAction SilentlyContinue).Product" 2^>nul') do set "mobo_model=%%i"
if not defined mobo_model for /f "tokens=1,* delims==" %%a in ('wmic baseboard get product /value ^| find "="') do for /f "delims=" %%i in ("%%b") do set "mobo_model=%%i"
...
set "_scsi_found="
for /f "delims=" %%j in ('powershell -NoProfile -Command "Get-CimInstance -ClassName Win32_SCSIController -ErrorAction SilentlyContinue | Where-Object { $_.PNPDeviceID -like 'PCI\VEN_*' } | Select-Object -ExpandProperty PNPDeviceID" 2^>nul') do (
    set "_scsi_found=1"
...
if not defined _scsi_found for /f "tokens=*" %%i in ('wmic path Win32_SCSIController get PNPDeviceID /format:value 2^>nul ^| find "PNPDeviceID=" ^| find "PCI\VEN_"') do (
    for /f "tokens=2 delims==" %%j in ("%%i") do (
        reg add "HKLM\SYSTEM\CurrentControlSet\Enum\%%j\Device Parameters\Interrupt Management\MessageSignaledInterruptProperties" /v "MSISupported" /t REG_DWORD /d "1" /f >nul 2>&1
        reg add "HKLM\SYSTEM\CurrentControlSet\Enum\%%j\Device Parameters" /v "ForceFifo" /t REG_DWORD /d "0" /f >nul 2>&1
    )
...
if not defined _ide_found for /f %%i in ('wmic path Win32_IDEController get PNPDeviceID 2^>nul ^| findstr /l "PCI\VEN_"') do (
    reg add "HKLM\SYSTEM\CurrentControlSet\Enum\%%i\Device Parameters\Interrupt Management\MessageSignaledInterruptProperties" /v "MSISupported" /t REG_DWORD /d "1" /f >nul 2>&1
    reg add "HKLM\SYSTEM\CurrentControlSet\Enum\%%i\Device Parameters" /v "ForceFifo" /t REG_DWORD /d "0" /f >nul 2>&1
) >nul 2>&1
...
for /f "tokens=1,* delims=|" %%i in ('powershell -NoProfile -Command "Get-CimInstance -ClassName Win32_DiskDrive -ErrorAction SilentlyContinue | ForEach-Object { $sz = 0; try { $sz = [math]::Round($_.Size/1GB,0) } catch {}; ('{0}|{1}' -f $_.Model, $sz) }" 2^>nul') do (
    if not "%%i"=="" (
        ...
        if !drivecount! equ 1 (
            set "drive1_name=%%i"
            if %%j gtr 0 set "drive1_size=%%j"
        )
        if !drivecount! equ 2 (
            set "drive2_name=%%i"
            if %%j gtr 0 set "drive2_size=%%j"
        )
...
:ManageMedia
call :ManageFeatureToggle "Media Features" "MediaPlayback"
goto CustomFeatures

:ManageNET
call :ManageFeatureToggle ".NET Framework 3.5" "NetFx3"
call :ManageFeatureToggle ".NET Framework 4 Advanced Services" "NetFx4-AdvSrvs"
goto CustomFeatures

:ManageIE
call :ManageFeatureToggle "Internet Explorer Optional Component" "Internet-Explorer-Optional-amd64"
goto CustomFeatures

:ManageContainers
call :ManageFeatureToggle "Windows Containers" "Containers"
goto CustomFeatures

:ManagePrint
call :ManageFeatureToggle "Printing Foundation Features" "Printing-Foundation-Features"
goto CustomFeatures

:ManageLegacy
call :ManageFeatureToggle "Legacy Components" "LegacyComponents"
call :ManageFeatureToggle "DirectPlay" "DirectPlay"
goto CustomFeatures
...
echo %c%[2] Disable !featureLabel!%u%
...
if !NumberOfCores! gtr 4 (
	set "_aff_vc_found="
	for /f "delims=" %%i in ('powershell -NoProfile -Command "Get-CimInstance -ClassName Win32_VideoController -ErrorAction SilentlyContinue | Where-Object { $_.PNPDeviceID -like 'PCI\VEN_*' } | Select-Object -ExpandProperty PNPDeviceID" 2^>nul') do (
		set "_aff_vc_found=1"
	...
	if not defined _aff_vc_found for /f %%i in ('wmic path Win32_VideoController get PNPDeviceID^| findstr /l "PCI\VEN_"') do (
		reg.exe add "HKLM\SYSTEM\CurrentControlSet\Enum\%%i\Device Parameters\Interrupt Management\Affinity Policy" /v "DevicePolicy" /t REG_DWORD /d "3" /f
		reg.exe delete "HKLM\SYSTEM\CurrentControlSet\Enum\%%i\Device Parameters\Interrupt Management\Affinity Policy" /v "AssignmentSetOverride" /f
	) > nul 2> nul
	set "_aff_na_found="
	for /f "delims=" %%i in ('powershell -NoProfile -Command "Get-CimInstance -ClassName Win32_NetworkAdapter -ErrorAction SilentlyContinue | Where-Object { $_.PNPDeviceID -like 'PCI\VEN_*' } | Select-Object -ExpandProperty PNPDeviceID" 2^>nul') do (
		set "_aff_na_found=1"
	...
	if not defined _aff_na_found for /f %%i in ('wmic path Win32_NetworkAdapter get PNPDeviceID^| findstr /l "PCI\VEN_"') do (
		reg.exe add "HKLM\SYSTEM\CurrentControlSet\Enum\%%i\Device Parameters\Interrupt Management\Affinity Policy" /v "DevicePolicy" /t REG_DWORD /d "5" /f
		reg.exe delete "HKLM\SYSTEM\CurrentControlSet\Enum\%%i\Device Parameters\Interrupt Management\Affinity Policy" /v "AssignmentSetOverride" /f
	) > nul 2> nul
)
...
if not defined _ht_usb_found for /f %%i in ('wmic path Win32_USBController get PNPDeviceID^| findstr /l "PCI\VEN_"') do (
	reg.exe add "HKLM\SYSTEM\CurrentControlSet\Enum\%%i\Device Parameters\Interrupt Management\Affinity Policy" /v "DevicePolicy" /t REG_DWORD /d "4" /f
	reg.exe add "HKLM\SYSTEM\CurrentControlSet\Enum\%%i\Device Parameters\Interrupt Management\Affinity Policy" /v "AssignmentSetOverride" /t REG_BINARY /d "C0" /f
	) > nul 2> nul
set "_ht_vc_found="
...
if not defined _ht_vc_found for /f %%i in ('wmic path Win32_VideoController get PNPDeviceID^| findstr /l "PCI\VEN_"') do (
	reg.exe add "HKLM\SYSTEM\CurrentControlSet\Enum\%%i\Device Parameters\Interrupt Management\Affinity Policy" /v "DevicePolicy" /t REG_DWORD /d "3" /f
	reg.exe delete "HKLM\SYSTEM\CurrentControlSet\Enum\%%i\Device Parameters\Interrupt Management\Affinity Policy" /v "AssignmentSetOverride" /f
	) > nul 2> nul
set "_ht_na_found="
...
if not defined _ht_na_found for /f %%i in ('wmic path Win32_NetworkAdapter get PNPDeviceID^| findstr /l "PCI\VEN_"') do (
	reg.exe add "HKLM\SYSTEM\CurrentControlSet\Enum\%%i\Device Parameters\Interrupt Management\Affinity Policy" /v "DevicePolicy" /t REG_DWORD /d "4" /f
	reg.exe add "HKLM\SYSTEM\CurrentControlSet\Enum\%%i\Device Parameters\Interrupt Management\Affinity Policy" /v "AssignmentSetOverride" /t REG_BINARY /d "30" /f
	) > nul 2> nul
)
...
if not defined _msi_usb_found for /f %%i in ('wmic path Win32_USBController get PNPDeviceID ^| findstr /l "PCI\VEN_"') do (
	reg.exe add "HKLM\SYSTEM\CurrentControlSet\Enum\%%i\Device Parameters\Interrupt Management\MessageSignaledInterruptProperties" /v "MSISupported" /t REG_DWORD /d "1" /f
	reg.exe delete "HKLM\SYSTEM\CurrentControlSet\Enum\%%i\Device Parameters\Interrupt Management\Affinity Policy" /v "DevicePriority" /f
) > nul 2> nul
...
if not defined _msi_vc_found for /f %%i in ('wmic path Win32_VideoController get PNPDeviceID ^| findstr /l "PCI\VEN_"') do (
	reg.exe add "HKLM\SYSTEM\CurrentControlSet\Enum\%%i\Device Parameters\Interrupt Management\MessageSignaledInterruptProperties" /v "MSISupported" /t REG_DWORD /d "1" /f
	reg.exe delete "HKLM\SYSTEM\CurrentControlSet\Enum\%%i\Device Parameters\Interrupt Management\Affinity Policy" /v "DevicePriority" /f
) > nul 2> nul
...
if not defined _msi_na_found for /f %%i in ('wmic path Win32_NetworkAdapter get PNPDeviceID ^| findstr /l "PCI\VEN_"') do (
	reg.exe add "HKLM\SYSTEM\CurrentControlSet\Enum\%%i\Device Parameters\Interrupt Management\MessageSignaledInterruptProperties" /v "MSISupported" /t REG_DWORD /d "1" /f
	reg.exe delete "HKLM\SYSTEM\CurrentControlSet\Enum\%%i\Device Parameters\Interrupt Management\Affinity Policy" /v "DevicePriority" /f
) > nul 2> nul
...
set "_msi_ide_found="
for /f "delims=" %%i in ('powershell -NoProfile -Command "Get-CimInstance -ClassName Win32_IDEController -ErrorAction SilentlyContinue | Where-Object { $_.PNPDeviceID -like 'PCI\VEN_*' } | Select-Object -ExpandProperty PNPDeviceID" 2^>nul') do (
	set "_msi_ide_found=1"
	reg.exe add "HKLM\SYSTEM\CurrentControlSet\Enum\%%i\Device Parameters\Interrupt Management\MessageSignaledInterruptProperties" /v "MSISupported" /t REG_DWORD /d "1" /f
	reg.exe delete "HKLM\SYSTEM\CurrentControlSet\Enum\%%i\Device Parameters\Interrupt Management\Affinity Policy" /v "DevicePriority" /f
) >nul 2>nul
if not defined _msi_ide_found for /f %%i in ('wmic path Win32_IDEController get PNPDeviceID ^| findstr /l "PCI\VEN_"') do (
	reg.exe add "HKLM\SYSTEM\CurrentControlSet\Enum\%%i\Device Parameters\Interrupt Management\MessageSignaledInterruptProperties" /v "MSISupported" /t REG_DWORD /d "1" /f
	reg.exe delete "HKLM\SYSTEM\CurrentControlSet\Enum\%%i\Device Parameters\Interrupt Management\Affinity Policy" /v "DevicePriority" /f
) > nul 2> nul
...
echo.
echo                 %c%Uses Chocolatey for 159 automatic packages plus 49 guided/manual entries.%u%
echo.
echo.
echo                                          %c%Press any key to start Dex Toolbox!%u%
pause >nul
...
echo.
echo  [U] Update all outdated Chocolatey packages
...
if errorlevel 1 (
    ...
    if errorlevel 1 (
        echo One or more updates failed. No checksum bypass was used.
        call :LogEvent "FAIL" "Chocolatey upgrade all returned an error"
    ) else (
        echo All available Chocolatey updates completed.
        call :LogEvent "OK" "Chocolatey packages updated"
    )
    pause
    goto SoftwareStatus
)
...
call :CreateManagedSnapshot "Smoke_Test"
if not exist "%DEX_LOG%" set "DEX_SMOKE_INVALID=1"
...
:InitializeDexRuntime
if defined DEX_TEST_ROOT (
    set "DEX_ROOT=%DEX_TEST_ROOT%"
) else (
    set "DEX_ROOT=%ProgramData%\DexTweaks"
)
set "DEX_LOG_DIR=%DEX_ROOT%\Logs"
set "DEX_BACKUPS=%DEX_ROOT%\Backups"
set "DEX_REPORTS=%DEX_ROOT%\Reports"
...
set "DEX_CATALOG=%DEX_STATE_DIR%\search_catalog.txt"
set "DEX_DESKTOP=%USERPROFILE%\Desktop"
...
:MarkRebootRequired
> "%DEX_REBOOT_FILE%" echo %date% %time% - %~1
set "DEX_REBOOT_STATUS=Required"
call :LogEvent "REBOOT" "%~1"
exit /b

:RequireExpertMode
echo.
echo %red%EXPERT MODE - HIGH RISK ACTION%u%
echo %~1
...
if errorlevel 1 (
    ...
    call :LogEvent "BACKUP" "Expert action Restore Point created"
)
call :LogEvent "EXPERT" "%~1"
exit /b 0
...
reg query "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\WindowsUpdate\Auto Update\RebootRequired" >nul 2>&1 && set "DEX_REBOOT_STATUS=Required"
exit /b
...
if errorlevel 3 goto HealthCenter
if errorlevel 2 goto BackupManager
if errorlevel 1 goto DashboardCenter
...
if errorlevel 4 (
    ...
    set "DEX_LAST_PROFILE=Not applied"
    call :LogEvent "INFO" "Cached state reset"
    goto SettingsCenter
)
if errorlevel 3 (
    start "" explorer.exe "%DEX_ROOT%"
    goto SettingsCenter
)
if errorlevel 2 (
    call :ExportDashboardReport
    goto SettingsCenter
)
...
call :LogEvent "REPORT" "Dashboard report created"
pause
exit /b
...
echo  [3] Competitive  High performance, HAGS and reduced visual effects
echo  [4] Streaming    Gaming and capture-friendly configuration
echo  [5] Laptop       Balanced power and battery-friendly behavior
echo  [6] Privacy      Reduced advertising and optional diagnostics
echo  [7] Import       Desktop\DexTweaks_Profile.queue
...
if "%DEX_PROFILE_CHOICE%"=="8" (
    call :ExportManagedProfile
    goto ProfilesCenter
)
if "%DEX_PROFILE_CHOICE%"=="7" (
    call :ImportManagedProfile
    if errorlevel 1 goto ProfilesCenter
    goto ReviewQueue
)
if "%DEX_PROFILE_CHOICE%"=="6" call :BuildProfile "Privacy"
if "%DEX_PROFILE_CHOICE%"=="5" call :BuildProfile "Laptop"
if "%DEX_PROFILE_CHOICE%"=="4" call :BuildProfile "Streaming"
if "%DEX_PROFILE_CHOICE%"=="3" call :BuildProfile "Competitive"
if "%DEX_PROFILE_CHOICE%"=="2" call :BuildProfile "Balanced"
if "%DEX_PROFILE_CHOICE%"=="1" call :BuildProfile "Safe"
goto ReviewQueue

:BuildProfile
set "DEX_SELECTED_PROFILE=%~1"
> "%DEX_QUEUE%" echo # Dex profile: %~1
if /I "%~1"=="Safe" (
    >>"%DEX_QUEUE%" echo DEFENDER_ON
    >>"%DEX_QUEUE%" echo UPDATES_ON
    >>"%DEX_QUEUE%" echo SEARCH_ON
    >>"%DEX_QUEUE%" echo PWR_BAL
    >>"%DEX_QUEUE%" echo VFX_AUTO
)
if /I "%~1"=="Balanced" (
    >>"%DEX_QUEUE%" echo DEFENDER_ON
    >>"%DEX_QUEUE%" echo UPDATES_ON
    >>"%DEX_QUEUE%" echo GM_ON
    >>"%DEX_QUEUE%" echo HAGS_ON
    >>"%DEX_QUEUE%" echo PWR_BAL
    >>"%DEX_QUEUE%" echo VFX_AUTO
)
if /I "%~1"=="Competitive" (
    ...
    >>"%DEX_QUEUE%" echo GM_ON
    >>"%DEX_QUEUE%" echo HAGS_ON
    >>"%DEX_QUEUE%" echo PWR_HIGH
    >>"%DEX_QUEUE%" echo VFX_PERF
    >>"%DEX_QUEUE%" echo DELIVERY_OFF
)
if /I "%~1"=="Streaming" (
    >>"%DEX_QUEUE%" echo DEFENDER_ON
    >>"%DEX_QUEUE%" echo GM_ON
...
if /I "%~1"=="Privacy" (
    >>"%DEX_QUEUE%" echo DEFENDER_ON
    >>"%DEX_QUEUE%" echo UPDATES_ON
    >>"%DEX_QUEUE%" echo TELEMETRY_SAFE
    >>"%DEX_QUEUE%" echo ADS_OFF
    >>"%DEX_QUEUE%" echo ACTIVITY_OFF
    >>"%DEX_QUEUE%" echo DELIVERY_OFF
)
call :LogEvent "QUEUE" "Profile %~1 prepared"
exit /b
...
:ApplyQueue
set /a DEX_APPLY_OK=0
set /a DEX_APPLY_FAIL=0
echo.
echo Applying queued changes...
copy /y "%DEX_QUEUE%" "%DEX_STATE_DIR%\last_applied.queue" >nul 2>&1
for /f "usebackq eol=# delims=" %%Q in ("%DEX_QUEUE%") do call :ApplyChange "%%Q"
del "%DEX_QUEUE%" >nul 2>&1
exit /b

:ExportManagedProfile
if not exist "%DEX_STATE_DIR%\last_applied.queue" (
    echo.
    echo No managed profile has been applied yet.
    pause
    exit /b 1
)
copy /y "%DEX_STATE_DIR%\last_applied.queue" "%DEX_DESKTOP%\DexTweaks_Profile.queue" >nul 2>&1
if errorlevel 1 (
    echo Could not export the profile to Desktop.
    call :LogEvent "FAIL" "Managed profile export failed"
) else (
    echo Profile exported to Desktop\DexTweaks_Profile.queue
    call :LogEvent "PROFILE" "Managed profile exported"
)
pause
exit /b

:ImportManagedProfile
set "DEX_IMPORT_FILE=%DEX_DESKTOP%\DexTweaks_Profile.queue"
if not exist "%DEX_IMPORT_FILE%" (
    echo.
    echo Import file not found: "%DEX_IMPORT_FILE%"
    pause
    exit /b 1
)
set "DEX_IMPORT_INVALID=0"
for /f "usebackq eol=# delims=" %%Q in ("%DEX_IMPORT_FILE%") do (
    call :ValidateChangeCode "%%Q"
    if errorlevel 1 set "DEX_IMPORT_INVALID=1"
)
if "%DEX_IMPORT_INVALID%"=="1" (
    echo.
    echo Import blocked: the profile contains an unknown action.
    call :LogEvent "BLOCKED" "Invalid managed profile import"
    pause
    exit /b 1
)
copy /y "%DEX_IMPORT_FILE%" "%DEX_QUEUE%" >nul 2>&1
set "DEX_SELECTED_PROFILE=Imported"
call :LogEvent "PROFILE" "Managed profile imported"
exit /b 0

:ValidateChangeCode
for %%C in (DEFENDER_ON UPDATES_ON SEARCH_ON SEARCH_OFF GM_ON HAGS_ON HAGS_OFF PWR_BAL PWR_HIGH VFX_AUTO VFX_PERF TELEMETRY_SAFE ADS_OFF ACTIVITY_OFF DELIVERY_OFF BGAPPS_OFF) do if /I "%~1"=="%%C" exit /b 0
exit /b 1
...
if /I "%DEX_CHANGE%"=="UPDATES_ON" (
    sc config wuauserv start= demand >nul 2>&1
    if errorlevel 1 set "DEX_CHANGE_OK=0"
    sc config BITS start= demand >nul 2>&1
)
if /I "%DEX_CHANGE%"=="SEARCH_ON" (
    sc config WSearch start= delayed-auto >nul 2>&1
    if errorlevel 1 sc config WSearch start= auto >nul 2>&1
    if errorlevel 1 set "DEX_CHANGE_OK=0"
)
if /I "%DEX_CHANGE%"=="SEARCH_OFF" (
    sc stop WSearch >nul 2>&1
    sc config WSearch start= disabled >nul 2>&1
    if errorlevel 1 set "DEX_CHANGE_OK=0"
)
if /I "%DEX_CHANGE%"=="GM_ON" (
    reg add "HKCU\Software\Microsoft\GameBar" /v AllowAutoGameMode /t REG_DWORD /d 1 /f >nul 2>&1
    if errorlevel 1 set "DEX_CHANGE_OK=0"
    reg add "HKCU\System\GameConfigStore" /v GameDVR_Enabled /t REG_DWORD /d 0 /f >nul 2>&1
    reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\GameDVR" /v AppCaptureEnabled /t REG_DWORD /d 0 /f >nul 2>&1
)
if /I "%DEX_CHANGE%"=="HAGS_ON" (
    reg add "HKLM\SYSTEM\CurrentControlSet\Control\GraphicsDrivers" /v HwSchMode /t REG_DWORD /d 2 /f >nul 2>&1
    if errorlevel 1 set "DEX_CHANGE_OK=0"
    call :MarkRebootRequired "HAGS setting changed"
)
if /I "%DEX_CHANGE%"=="HAGS_OFF" (
    reg add "HKLM\SYSTEM\CurrentControlSet\Control\GraphicsDrivers" /v HwSchMode /t REG_DWORD /d 1 /f >nul 2>&1
    if errorlevel 1 set "DEX_CHANGE_OK=0"
    call :MarkRebootRequired "HAGS setting changed"
)
if /I "%DEX_CHANGE%"=="PWR_BAL" (
    powercfg /setactive SCHEME_BALANCED >nul 2>&1
    if errorlevel 1 set "DEX_CHANGE_OK=0"
)
if /I "%DEX_CHANGE%"=="PWR_HIGH" (
    powercfg /setactive SCHEME_MIN >nul 2>&1
    if errorlevel 1 set "DEX_CHANGE_OK=0"
)
if /I "%DEX_CHANGE%"=="VFX_AUTO" (
    reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\Explorer\VisualEffects" /v VisualFXSetting /t REG_DWORD /d 0 /f >nul 2>&1
    if errorlevel 1 set "DEX_CHANGE_OK=0"
)
if /I "%DEX_CHANGE%"=="VFX_PERF" (
    reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\Explorer\VisualEffects" /v VisualFXSetting /t REG_DWORD /d 2 /f >nul 2>&1
    if errorlevel 1 set "DEX_CHANGE_OK=0"
)
if /I "%DEX_CHANGE%"=="TELEMETRY_SAFE" (
    reg add "HKLM\SOFTWARE\Policies\Microsoft\Windows\DataCollection" /v AllowTelemetry /t REG_DWORD /d 1 /f >nul 2>&1
    if errorlevel 1 set "DEX_CHANGE_OK=0"
)
if /I "%DEX_CHANGE%"=="ADS_OFF" (
    reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\AdvertisingInfo" /v Enabled /t REG_DWORD /d 0 /f >nul 2>&1
    if errorlevel 1 set "DEX_CHANGE_OK=0"
)
if /I "%DEX_CHANGE%"=="ACTIVITY_OFF" (
    reg add "HKLM\SOFTWARE\Policies\Microsoft\Windows\System" /v EnableActivityFeed /t REG_DWORD /d 0 /f >nul 2>&1
    if errorlevel 1 set "DEX_CHANGE_OK=0"
    reg add "HKLM\SOFTWARE\Policies\Microsoft\Windows\System" /v PublishUserActivities /t REG_DWORD /d 0 /f >nul 2>&1
    reg add "HKLM\SOFTWARE\Policies\Microsoft\Windows\System" /v UploadUserActivities /t REG_DWORD /d 0 /f >nul 2>&1
)
if /I "%DEX_CHANGE%"=="DELIVERY_OFF" (
    reg add "HKLM\SOFTWARE\Policies\Microsoft\Windows\DeliveryOptimization" /v DODownloadMode /t REG_DWORD /d 0 /f >nul 2>&1
    if errorlevel 1 set "DEX_CHANGE_OK=0"
)
if /I "%DEX_CHANGE%"=="BGAPPS_OFF" (
    reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\BackgroundAccessApplications" /v GlobalUserDisabled /t REG_DWORD /d 1 /f >nul 2>&1
    if errorlevel 1 set "DEX_CHANGE_OK=0"
)
if "%DEX_CHANGE_OK%"=="1" (
    call :VerifyManagedChange "%DEX_CHANGE%"
    if errorlevel 1 set "DEX_CHANGE_OK=0"
)
if "%DEX_CHANGE_OK%"=="1" (
    echo   Result: applied and state verified
    set /a DEX_APPLY_OK+=1
    call :LogEvent "OK" "Applied %DEX_CHANGE%"
) else (
    echo   Result: failed or blocked
    set /a DEX_APPLY_FAIL+=1
    call :LogEvent "FAIL" "Could not apply %DEX_CHANGE%"
)
exit /b

:VerifyManagedChange
if /I "%~1"=="DEFENDER_ON" (
    powershell -NoProfile -Command "$s=Get-MpComputerStatus -ErrorAction SilentlyContinue;if($s.AntivirusEnabled -and $s.RealTimeProtectionEnabled){exit 0}else{exit 1}" >nul 2>&1
    exit /b !errorlevel!
)
if /I "%~1"=="UPDATES_ON" (
    powershell -NoProfile -Command "$a=(Get-Service wuauserv -ErrorAction SilentlyContinue).StartType;$b=(Get-Service BITS -ErrorAction SilentlyContinue).StartType;if($a-ne'Disabled'-and$b-ne'Disabled'){exit 0}else{exit 1}" >nul 2>&1
    exit /b !errorlevel!
)
if /I "%~1"=="SEARCH_ON" (
    powershell -NoProfile -Command "if((Get-Service WSearch -ErrorAction SilentlyContinue).StartType-ne'Disabled'){exit 0}else{exit 1}" >nul 2>&1
    exit /b !errorlevel!
)
if /I "%~1"=="SEARCH_OFF" (
    powershell -NoProfile -Command "if((Get-Service WSearch -ErrorAction SilentlyContinue).StartType-eq'Disabled'){exit 0}else{exit 1}" >nul 2>&1
    exit /b !errorlevel!
)
if /I "%~1"=="GM_ON" (
    reg query "HKCU\Software\Microsoft\GameBar" /v AllowAutoGameMode 2>nul ^| findstr /I "0x1" >nul
    exit /b !errorlevel!
)
if /I "%~1"=="HAGS_ON" (
...
if /I "%~1"=="HAGS_OFF" (
    reg query "HKLM\SYSTEM\CurrentControlSet\Control\GraphicsDrivers" /v HwSchMode 2>nul ^| findstr /I "0x1" >nul
    exit /b !errorlevel!
)
if /I "%~1"=="PWR_BAL" (
    powercfg /getactivescheme 2>nul ^| findstr /I "381b4222-f694-41f0-9685-ff5bb260df2e" >nul
    exit /b !errorlevel!
)
if /I "%~1"=="PWR_HIGH" (
...
if /I "%~1"=="VFX_AUTO" (
    reg query "HKCU\Software\Microsoft\Windows\CurrentVersion\Explorer\VisualEffects" /v VisualFXSetting 2>nul ^| findstr /I "0x0" >nul
    exit /b !errorlevel!
)
if /I "%~1"=="VFX_PERF" (
...
if /I "%~1"=="TELEMETRY_SAFE" (
    reg query "HKLM\SOFTWARE\Policies\Microsoft\Windows\DataCollection" /v AllowTelemetry 2>nul ^| findstr /I "0x1" >nul
    exit /b !errorlevel!
)
if /I "%~1"=="ADS_OFF" (
    reg query "HKCU\Software\Microsoft\Windows\CurrentVersion\AdvertisingInfo" /v Enabled 2>nul ^| findstr /I "0x0" >nul
    exit /b !errorlevel!
)
if /I "%~1"=="ACTIVITY_OFF" (
    reg query "HKLM\SOFTWARE\Policies\Microsoft\Windows\System" /v EnableActivityFeed 2>nul ^| findstr /I "0x0" >nul
    exit /b !errorlevel!
)
if /I "%~1"=="DELIVERY_OFF" (
    reg query "HKLM\SOFTWARE\Policies\Microsoft\Windows\DeliveryOptimization" /v DODownloadMode 2>nul ^| findstr /I "0x0" >nul
    exit /b !errorlevel!
)
if /I "%~1"=="BGAPPS_OFF" (
    reg query "HKCU\Software\Microsoft\Windows\CurrentVersion\BackgroundAccessApplications" /v GlobalUserDisabled 2>nul ^| findstr /I "0x1" >nul
    exit /b !errorlevel!
)
exit /b 1

:ProfileComplete
if %DEX_APPLY_FAIL% EQU 0 (
    set "DEX_LAST_PROFILE=%DEX_SELECTED_PROFILE%"
...
echo.
echo  Profile: %DEX_SELECTED_PROFILE%
echo  Successful actions: %DEX_APPLY_OK%
echo  Failed actions: %DEX_APPLY_FAIL%
echo  Reboot: %DEX_REBOOT_STATUS%
echo.
echo Full details: "%DEX_LOG%"
echo.
pause
goto menu
...
echo.
echo  [1] Enable Game Mode and disable background capture
echo  [2] Enable HAGS                 [3] Disable HAGS
echo  [4] Balanced power             [5] High Performance power
echo  [6] Enable Windows Search      [7] Disable Windows Search
echo  [8] Restore Defender protection
echo  [9] Restore latest managed snapshot
...
if "%DEX_CHANGE_CHOICE%"=="9" (
    call :RestoreLatestManagedSnapshot
    goto ChangeCenter
)
set "DEX_SELECTED_PROFILE=Custom"
> "%DEX_QUEUE%" echo # Custom managed change
if "%DEX_CHANGE_CHOICE%"=="8" >>"%DEX_QUEUE%" echo DEFENDER_ON
if "%DEX_CHANGE_CHOICE%"=="7" >>"%DEX_QUEUE%" echo SEARCH_OFF
if "%DEX_CHANGE_CHOICE%"=="6" >>"%DEX_QUEUE%" echo SEARCH_ON
if "%DEX_CHANGE_CHOICE%"=="5" >>"%DEX_QUEUE%" echo PWR_HIGH
if "%DEX_CHANGE_CHOICE%"=="4" >>"%DEX_QUEUE%" echo PWR_BAL
if "%DEX_CHANGE_CHOICE%"=="3" >>"%DEX_QUEUE%" echo HAGS_OFF
if "%DEX_CHANGE_CHOICE%"=="2" >>"%DEX_QUEUE%" echo HAGS_ON
if "%DEX_CHANGE_CHOICE%"=="1" >>"%DEX_QUEUE%" echo GM_ON
goto ReviewQueue

:CollectManagedChangeState
set "DEX_GM_STATE=Off"
reg query "HKCU\Software\Microsoft\GameBar" /v AllowAutoGameMode 2>nul | findstr /I "0x1" >nul && set "DEX_GM_STATE=On"
set "DEX_HAGS_STATE=Default"
reg query "HKLM\SYSTEM\CurrentControlSet\Control\GraphicsDrivers" /v HwSchMode 2>nul | findstr /I "0x2" >nul && set "DEX_HAGS_STATE=On"
reg query "HKLM\SYSTEM\CurrentControlSet\Control\GraphicsDrivers" /v HwSchMode 2>nul | findstr /I "0x1" >nul && set "DEX_HAGS_STATE=Off"
set "DEX_SEARCH_STATE=Unknown"
for /f "delims=" %%S in ('powershell -NoProfile -Command "try{(Get-Service WSearch).StartType}catch{'Unavailable'}" 2^>nul') do set "DEX_SEARCH_STATE=%%S"
call :RefreshDashboardCache
exit /b
...
echo.
echo  [1] Game Mode on        [2] HAGS on           [3] HAGS off
echo  [4] Balanced power      [5] High Performance  [6] Search on
echo  [7] Search off          [8] Defender on
echo  [A] Review and apply    [0] Cancel
echo.
echo Current queue:
...
if "%DEX_QUEUE_CHOICE%"=="8" >>"%DEX_QUEUE%" echo DEFENDER_ON
if "%DEX_QUEUE_CHOICE%"=="7" >>"%DEX_QUEUE%" echo SEARCH_OFF
if "%DEX_QUEUE_CHOICE%"=="6" >>"%DEX_QUEUE%" echo SEARCH_ON
if "%DEX_QUEUE_CHOICE%"=="5" >>"%DEX_QUEUE%" echo PWR_HIGH
if "%DEX_QUEUE_CHOICE%"=="4" >>"%DEX_QUEUE%" echo PWR_BAL
if "%DEX_QUEUE_CHOICE%"=="3" >>"%DEX_QUEUE%" echo HAGS_OFF
if "%DEX_QUEUE_CHOICE%"=="2" >>"%DEX_QUEUE%" echo HAGS_ON
if "%DEX_QUEUE_CHOICE%"=="1" >>"%DEX_QUEUE%" echo GM_ON
goto CustomQueueBuilderLoop
...
reg export "HKLM\SYSTEM\CurrentControlSet\Control\GraphicsDrivers" "%DEX_SNAPSHOT%\GraphicsDrivers.reg" /y >nul 2>&1
reg export "HKCU\Software\Microsoft\Windows\CurrentVersion\Explorer\VisualEffects" "%DEX_SNAPSHOT%\VisualEffects.reg" /y >nul 2>&1
reg export "HKLM\SOFTWARE\Policies\Microsoft\Windows" "%DEX_SNAPSHOT%\WindowsPolicies.reg" /y >nul 2>&1
reg export "HKCU\Software\Microsoft\Windows\CurrentVersion\AdvertisingInfo" "%DEX_SNAPSHOT%\AdvertisingInfo.reg" /y >nul 2>&1
powercfg /getactivescheme > "%DEX_SNAPSHOT%\power.txt" 2>nul
bcdedit /export "%DEX_SNAPSHOT%\bcd_store.bak" >nul 2>&1
powershell -NoProfile -Command "Get-MpPreference -ErrorAction SilentlyContinue | Select-Object DisableRealtimeMonitoring,DisableBehaviorMonitoring,DisableBlockAtFirstSeen,DisableIOAVProtection,DisableScriptScanning,MAPSReporting,SubmitSamplesConsent,ExclusionPath,ExclusionProcess | ConvertTo-Json -Depth 4 | Set-Content -Encoding UTF8 -LiteralPath '%DEX_SNAPSHOT%\defender.json'" >nul 2>&1
...
if not exist "%DEX_SNAPSHOT_FILE%" (
    echo.
    echo No managed snapshot is registered.
    pause
    exit /b
)
set "DEX_RESTORE_PATH="
set /p DEX_RESTORE_PATH=<"%DEX_SNAPSHOT_FILE%"
if not defined DEX_RESTORE_PATH (
    echo Invalid snapshot state.
    pause
    exit /b
)
call :ValidateManagedSnapshotPath
...
echo  [3] Create Windows Restore Point
echo  [4] Create full registry backup
echo  [5] List available Dex backups
echo  [6] Open Windows System Restore
echo  [7] Selective restore by module
...
if errorlevel 4 (
    call :CreateFullRegistryBackup
    goto BackupManager
)
if errorlevel 3 (
    call :CreateWindowsRestorePoint
    goto BackupManager
)
if errorlevel 2 (
    call :RestoreLatestManagedSnapshot
...
if errorlevel 1 (
    call :CreateManagedSnapshot "Manual"
    pause
    goto BackupManager
)
goto BackupManager

:ChooseManagedSnapshot
echo.
echo Available managed snapshots:
dir /b /ad /o-d "%DEX_BACKUPS%\Managed_*" 2>nul
if errorlevel 1 (
    echo No managed snapshots found.
    pause
    exit /b 1
)
echo.
set "DEX_PICK_SNAPSHOT="
set /p "DEX_PICK_SNAPSHOT=Type the complete folder name, or press Enter to cancel: "
if not defined DEX_PICK_SNAPSHOT exit /b 1
echo(!DEX_PICK_SNAPSHOT!| findstr /R /X /C:"Managed_[A-Za-z0-9_-][A-Za-z0-9_-]*" >nul
if errorlevel 1 (
    echo Invalid snapshot folder name.
    pause
    exit /b 1
)
if not exist "%DEX_BACKUPS%\!DEX_PICK_SNAPSHOT!\metadata.txt" (
    echo Snapshot metadata was not found.
    pause
    exit /b 1
)
>"%DEX_SNAPSHOT_FILE%" echo %DEX_BACKUPS%\!DEX_PICK_SNAPSHOT!
echo Selected snapshot: !DEX_PICK_SNAPSHOT!
call :LogEvent "BACKUP" "Selected managed snapshot !DEX_PICK_SNAPSHOT!"
pause
exit /b 0

:SelectiveRestoreLatest
if not exist "%DEX_SNAPSHOT_FILE%" (
    echo No managed snapshot is registered.
    pause
    exit /b
...
if "%DEX_RESTORE_MODULE%"=="3" call :RestoreSnapshotPower
if "%DEX_RESTORE_MODULE%"=="4" call :RestoreSnapshotDefender
if "%DEX_RESTORE_MODULE%"=="5" call :RestoreSnapshotBcd
if errorlevel 1 (
    echo Module restore failed or its backup file was unavailable.
    call :LogEvent "FAIL" "Selective snapshot restore module %DEX_RESTORE_MODULE%"
) else (
    echo Selected module restored successfully.
    call :MarkRebootRequired "Selective snapshot restore completed"
    call :LogEvent "RESTORE" "Selective snapshot restore module %DEX_RESTORE_MODULE%"
)
pause
exit /b
...
if "%DEX_MODULE_FOUND%"=="0" exit /b 1
exit /b 0
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "try { Enable-ComputerRestore -Drive $env:SystemDrive -ErrorAction SilentlyContinue; Checkpoint-Computer -Description 'DexTweaks_%DEX_SESSION%' -RestorePointType MODIFY_SETTINGS -ErrorAction Stop; exit 0 } catch { Write-Host $_.Exception.Message; exit 1 }"
if errorlevel 1 (
    echo Restore Point creation failed. Check System Protection settings.
    call :LogEvent "FAIL" "Windows Restore Point creation failed"
) else (
    echo Restore Point created successfully.
    call :LogEvent "BACKUP" "Windows Restore Point created"
)
pause
exit /b
...
echo.
echo Saving registry hives. This can take a few minutes...
set /a DEX_FULL_FAIL=0
REG SAVE HKLM\SOFTWARE "%DEX_FULL_PATH%\SOFTWARE.hiv" /y >nul 2>&1
if errorlevel 1 set /a DEX_FULL_FAIL+=1
REG SAVE HKLM\SYSTEM "%DEX_FULL_PATH%\SYSTEM.hiv" /y >nul 2>&1
if errorlevel 1 set /a DEX_FULL_FAIL+=1
REG SAVE HKU\.DEFAULT "%DEX_FULL_PATH%\DEFAULT.hiv" /y >nul 2>&1
if errorlevel 1 set /a DEX_FULL_FAIL+=1
REG SAVE HKLM\SECURITY "%DEX_FULL_PATH%\SECURITY.hiv" /y >nul 2>&1
if errorlevel 1 set /a DEX_FULL_FAIL+=1
REG SAVE HKLM\SAM "%DEX_FULL_PATH%\SAM.hiv" /y >nul 2>&1
if errorlevel 1 set /a DEX_FULL_FAIL+=1
...
:ListDexBackups
echo.
echo Managed backup folders:
dir /b /ad /o-d "%DEX_BACKUPS%" 2>nul
...
if errorlevel 4 (
    call :CriticalEventReport
    goto HealthCenter
)
if errorlevel 3 (
    call :StorageHealthReport
    goto HealthCenter
)
if errorlevel 2 (
    call :FullWindowsRepair
    goto HealthCenter
)
if errorlevel 1 (
...
:QuickHealthCheck
set "DEX_HEALTH_REPORT=%DEX_REPORTS%\Health_%DEX_SESSION%.txt"
echo.
echo Running component store check...
> "%DEX_HEALTH_REPORT%" echo Dex Tweaks Quick Health Check - %date% %time%
DISM /Online /Cleanup-Image /CheckHealth >>"%DEX_HEALTH_REPORT%" 2>&1
set "DEX_DISM_RC=%errorlevel%"
echo Verifying protected system files...
sfc /verifyonly >>"%DEX_HEALTH_REPORT%" 2>&1
set "DEX_SFC_RC=%errorlevel%"
echo.
echo DISM result code: %DEX_DISM_RC%
echo SFC result code: %DEX_SFC_RC%
echo Report: "%DEX_HEALTH_REPORT%"
call :LogEvent "HEALTH" "Quick check DISM=%DEX_DISM_RC% SFC=%DEX_SFC_RC%"
pause
exit /b
...
set "DEX_DISM_RC=%errorlevel%"
sfc /scannow >>"%DEX_REPAIR_REPORT%" 2>&1
set "DEX_SFC_RC=%errorlevel%"
echo Repair completed. DISM=%DEX_DISM_RC% SFC=%DEX_SFC_RC%
echo Report: "%DEX_REPAIR_REPORT%"
call :MarkRebootRequired "Windows component repair completed"
call :LogEvent "HEALTH" "Full repair DISM=%DEX_DISM_RC% SFC=%DEX_SFC_RC%"
pause
exit /b

:StorageHealthReport
set "DEX_STORAGE_REPORT=%DEX_REPORTS%\Storage_%DEX_SESSION%.txt"
powershell -NoProfile -Command "Get-PhysicalDisk -ErrorAction SilentlyContinue | Select-Object FriendlyName,MediaType,HealthStatus,OperationalStatus,Size | Format-Table -AutoSize | Out-String -Width 220 | Set-Content -Encoding UTF8 -LiteralPath '%DEX_STORAGE_REPORT%'; Get-Volume -ErrorAction SilentlyContinue | Select-Object DriveLetter,FileSystemLabel,FileSystem,HealthStatus,SizeRemaining,Size | Format-Table -AutoSize | Out-String -Width 220 | Add-Content -Encoding UTF8 -LiteralPath '%DEX_STORAGE_REPORT%'"
type "%DEX_STORAGE_REPORT%"
echo Report: "%DEX_STORAGE_REPORT%"
call :LogEvent "REPORT" "Storage health report created"
pause
exit /b

:CriticalEventReport
set "DEX_EVENT_REPORT=%DEX_REPORTS%\CriticalEvents_%DEX_SESSION%.txt"
powershell -NoProfile -Command "$start=(Get-Date).AddDays(-7); Get-WinEvent -FilterHashtable @{LogName='System';Level=1,2;StartTime=$start} -MaxEvents 100 -ErrorAction SilentlyContinue | Select-Object TimeCreated,Id,ProviderName,LevelDisplayName,Message | Format-List | Out-String -Width 220 | Set-Content -Encoding UTF8 -LiteralPath '%DEX_EVENT_REPORT%'"
echo Recent critical and error events saved to:
echo "%DEX_EVENT_REPORT%"
call :LogEvent "REPORT" "Critical event report created"
pause
exit /b

:SecurityAudit
set "DEX_SECURITY_REPORT=%DEX_REPORTS%\Security_%DEX_SESSION%.txt"
powershell -NoProfile -Command "$d=Get-MpComputerStatus -ErrorAction SilentlyContinue; $fw=Get-NetFirewallProfile -ErrorAction SilentlyContinue; try{$sb=Confirm-SecureBootUEFI}catch{$sb='Unavailable'}; [pscustomobject]@{AntivirusEnabled=$d.AntivirusEnabled;RealTimeProtection=$d.RealTimeProtectionEnabled;BehaviorMonitor=$d.BehaviorMonitorEnabled;IoavProtection=$d.IoavProtectionEnabled;FirewallProfiles=($fw | ForEach-Object { $_.Name+':'+$_.Enabled }) -join ', ';SecureBoot=$sb;TpmReady=(Get-Tpm -ErrorAction SilentlyContinue).TpmReady} | Format-List | Out-String | Set-Content -Encoding UTF8 -LiteralPath '%DEX_SECURITY_REPORT%'"
type "%DEX_SECURITY_REPORT%"
echo Report: "%DEX_SECURITY_REPORT%"
call :LogEvent "REPORT" "Security audit created"
pause
exit /b
...
echo  [3] Compare baseline with current result
...
if errorlevel 1 (
    ...
    echo Saved to: "%~1"
    call :LogEvent "BENCH" "Benchmark %~2 captured"
)
pause
exit /b

:CompareBenchmarks
if not exist "%DEX_BASELINE%" (
    echo Capture a baseline first.
    pause
    exit /b
)
if not exist "%DEX_AFTER%" (
    echo Capture the current result first.
    pause
    exit /b
)
echo.
powershell -NoProfile -Command "$a=@{};$b=@{};Get-Content -LiteralPath '%DEX_BASELINE%'|ForEach-Object{if($_ -match '^([^=]+)=(.*)$'){$a[$matches[1]]=$matches[2]}};Get-Content -LiteralPath '%DEX_AFTER%'|ForEach-Object{if($_ -match '^([^=]+)=(.*)$'){$b[$matches[1]]=$matches[2]}}; 'Metric                 Baseline       Current        Delta'; '------------------------------------------------------------'; foreach($k in @('ProcessCount','RunningServices','FreeMemoryMB','CpuLoadPercent','SystemDriveFreeGB')){$x=[double]$a[$k];$y=[double]$b[$k];'{0,-22}{1,-15}{2,-15}{3}' -f $k,$x,$y,[math]::Round($y-$x,2)}"
echo.
echo Measurements are snapshots, not guaranteed FPS or latency gains.
call :LogEvent "BENCH" "Benchmark comparison viewed"
pause
exit /b

:ViewBenchmarkFiles
echo.
if exist "%DEX_BASELINE%" (
    echo --- BASELINE ---
    type "%DEX_BASELINE%"
) else echo No baseline captured.
echo.
if exist "%DEX_AFTER%" (
    echo --- CURRENT ---
    type "%DEX_AFTER%"
) else echo No current result captured.
echo.
pause
exit /b
...
>>"%DEX_CATALOG%" echo PROF Profiles - safe, balanced, competitive, streaming, laptop and privacy
>>"%DEX_CATALOG%" echo CHANGE Change Center - Game Mode, HAGS, power, Search and Defender
>>"%DEX_CATALOG%" echo BACKUP Backup Restore - snapshot, registry and Restore Point
>>"%DEX_CATALOG%" echo HEALTH Health - DISM, SFC, disk, events and security
>>"%DEX_CATALOG%" echo BENCH Benchmark - baseline and current comparison
>>"%DEX_CATALOG%" echo LOG History - session logs and exports
>>"%DEX_CATALOG%" echo RESTART Restart - pending state, schedule and cancel
exit /b
...
echo.
set "DEX_SEARCH_TERM="
set /p "DEX_SEARCH_TERM=Search: "
if not defined DEX_SEARCH_TERM goto menu
echo(!DEX_SEARCH_TERM!| findstr /R /X /C:"[A-Za-z0-9 _-][A-Za-z0-9 _-]*" >nul
if errorlevel 1 (
    echo Invalid search. Use letters, numbers, spaces, underscore or hyphen.
    pause
    goto GlobalSearch
)
echo.
findstr /I /L /C:"!DEX_SEARCH_TERM!" "%DEX_CATALOG%"
if errorlevel 1 echo No matching section found.
echo.
echo Enter the code shown at the start of a result, or 0 to return.
set "DEX_SEARCH_CODE="
set /p "DEX_SEARCH_CODE=Code: "
...
if /I "!DEX_SEARCH_CODE!"=="PROF" goto ProfilesCenter
if /I "!DEX_SEARCH_CODE!"=="CHANGE" goto ChangeCenter
if /I "!DEX_SEARCH_CODE!"=="BACKUP" goto BackupManager
if /I "!DEX_SEARCH_CODE!"=="HEALTH" goto HealthCenter
if /I "!DEX_SEARCH_CODE!"=="BENCH" goto BenchmarkCenter
if /I "!DEX_SEARCH_CODE!"=="LOG" goto HistoryCenter
if /I "!DEX_SEARCH_CODE!"=="RESTART" goto RestartCenter
echo Invalid code.
pause
goto GlobalSearch
...
if errorlevel 3 (
    copy /y "%DEX_LOG%" "%DEX_DESKTOP%\DexTweaks_%DEX_SESSION%.log" >nul
    echo Exported to Desktop\DexTweaks_%DEX_SESSION%.log
    pause
    goto HistoryCenter
)
if errorlevel 2 (
    dir /b /o-d "%DEX_LOG_DIR%\*.log" 2>nul
    pause
    goto HistoryCenter
)
if errorlevel 1 (
...
echo.
echo  [1] Restart now
echo  [2] Schedule restart in 60 seconds
...
if errorlevel 4 (
    ...
    call :LogEvent "REBOOT" "Dex restart reminder cleared"
    goto RestartCenter
)
if errorlevel 3 (
    shutdown /a
    call :LogEvent "REBOOT" "Scheduled restart cancelled"
    pause
    goto RestartCenter
)
if errorlevel 2 (
    shutdown /r /t 60 /c "Dex Tweaks: restart scheduled to complete system changes."
    call :LogEvent "REBOOT" "Restart scheduled in 60 seconds"
    pause
    goto RestartCenter
)
if errorlevel 1 (
